...
1. **メタデータ生成**: 各ページで `Metadata API` が実行され、OGP画像URLとして `/api/og?path=/target-path` を返します。必要に応じて追加のクエリパラメータを付与することも可能です（例: `&theme=dark`）。
2. **画像リクエスト**: SNSやクローラーがこのURLにアクセスします。
3. **HTML取得**: `/api/og` エンドポイントがターゲットパスを取得し、付随するクエリパラメータも維持した状態で HTML を `fetch` します。
